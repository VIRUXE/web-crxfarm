...
@section('content')
    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
        <nav class="flex items-center gap-1.5 text-xs text-zinc-500 min-w-0" aria-label="Breadcrumb">
            <a href="{{ route('catalog.index') }}" class="hover:text-brand transition shrink-0">Inventory</a>
            <x-lucide-chevron-right class="size-3.5 text-zinc-400 shrink-0" />
            @if($listing->category)
                <a href="{{ route('catalog.index', ['category' => $listing->category->value]) }}" class="hover:text-brand transition shrink-0">{{ $listing->category->label() }}</a>
                <x-lucide-chevron-right class="size-3.5 text-zinc-400 shrink-0" />
            @elseif($listing->chassisLabel())
                <a href="{{ route('catalog.index', ['chassis' => $listing->isCar() ? $listing->chassis : $listing->compatibleChassis->first()?->name]) }}" class="hover:text-brand transition shrink-0">{{ $listing->chassisLabel() }}</a>
                <x-lucide-chevron-right class="size-3.5 text-zinc-400 shrink-0" />
            @endif
            <span class="truncate font-medium text-zinc-800" aria-current="page">{{ $listing->title }}</span>
        </nav>
